...
Class diagrams are orthogonal-only. The requirements below apply to orthogonal connector text placement.
...
### Line Exit Points
Orthogonal connectors should use the edge selected by the orthogonal planner so the first and final segments stay axis-aligned.

### Text Offset Calculation
Text offset should follow the active orthogonal segment so labels and multiplicities stay readable without introducing non-axis-aligned geometry.
